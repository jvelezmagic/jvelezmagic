...
corresponding word form with `unnest_tokens()`. This generated me a new column called `word`.
...
lexicon as the source from which they were extracted. Since the our data and the data generated by `get_stopwords()` contain the `word` column, I performed an
